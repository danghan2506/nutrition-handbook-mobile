...
export const AnalyticsSegmentControl: React.FC<AnalyticsSegmentControlProps> = ({
  ...
  return (
    <View className="flex-row bg-[#F0EAE1] p-1 rounded-2xl mx-4 my-3">
      <Pressable
        onPress={() => onSelectTab('overview')}
        accessibilityRole="tab"
        accessibilityState={{ selected: activeTab === 'overview' }}
        accessibilityLabel="Tổng Quan"
        className={`flex-1 py-3.5 rounded-xl items-center justify-center min-h-[44px] ${
          activeTab === 'overview' ? 'bg-[#FF9E7A]' : 'bg-transparent'
        }`}
      >
        <Text
      ...
      <Pressable
        ...
        className={`flex-1 py-3.5 rounded-xl items-center justify-center min-h-[44px] ${
          activeTab === 'details' ? 'bg-[#FF9E7A]' : 'bg-transparent'
        }`}
      >
        <Text
          className={`font-semibold text-sm ${
